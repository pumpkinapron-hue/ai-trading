...
`is_market_open` は祝日クローズ（元日・クリスマス・グッドフライデー）も見る。
以前は曜日しか見ておらず、祝日が丸1日=1440分の欠損として毎回出て
`longest_gap_minutes` がそれで飽和していた（10年で20件前後）。

ただし祝日表は近似（NYのローカル暦日で3日だけ）なので、`longest_gap_minutes` を
単独の見出し数値に使わない方針は変えない。`format_summary` が長い欠損とそれ以外を
分けて出すのは、祝日表に載っていない休場（各国の単発の祝日、障害）でも同じことが
起きるため。
"""
...
#: 「まる1日規模」とみなす欠損の長さ（分）。主要な祝日は `is_market_open` が
#: 見るようになったが、祝日表は近似（元日・クリスマス・グッドフライデーの3日だけ）
#: なので、表に載っていない休場——各国の単発の祝日、配信側の長時間障害——は
#: いまも丸1日規模の欠損として出る。この定数は「その規模を下回る欠損＝市場が
#: 開いているはずなのにデータが無い、調査対象になりうる欠損」を見出しの数字から
#: 拾うための表示上の閾値であり、祝日カレンダーの代用ではない。
...
def format_summary(report: QualityReport) -> str:
    ...
    主要な祝日は見るようになったが、表に載っていない休場（各国の単発の祝日、配信側の
    長時間障害）は丸1日規模の欠損として出る。単独の見出し数値として使うと、
